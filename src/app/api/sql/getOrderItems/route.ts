...
const computeOptionsExtra = (
    selectedOptionsRaw: string | null,
    allOptionsRaw: string | null
): { extra: number; desc: string[] } => {
    if (!selectedOptionsRaw) return { extra: 0, desc: [] };
    try {
        const sel: { type: string; valeur: string }[] = JSON.parse(selectedOptionsRaw);
        const def: { type: string; options: { valeur: string; prix: number | string }[] }[] = allOptionsRaw
            ? JSON.parse(allOptionsRaw)
            : [];
        let extra = 0;
        const desc: string[] = [];
        for (const s of sel) {
