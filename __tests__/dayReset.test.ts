import { computeResetTimes } from '@/app/contexts/DataProvider';
...
describe('Day Reset Logic', () => {
    ...
    describe('computeResetTimes', () => {
        describe('last reset time', () => {
            it('should return today closing hour if current time is after closing hour', () => {
                ...
                const { last } = computeResetTimes(closingHour, now);

                const expectedResetTime = new Date('2026-03-23T03:00:00').getTime();
                expect(last).toBe(expectedResetTime);
            });
        ...
        describe('next reset time', () => {
            it('should return tomorrow closing hour if current time is after today closing hour', () => {
                ...
                const { next } = computeResetTimes(closingHour, now);

                const expectedResetTime = new Date('2026-03-24T03:00:00').getTime();
                expect(next).toBe(expectedResetTime);
            });
        ...
        it('returns both last and next reset times together', () => {
            const now = new Date('2026-03-23T09:00:00');
            const closingHour = 3;

            const result = computeResetTimes(closingHour, now);

            expect(result.last).toBe(new Date('2026-03-23T03:00:00').getTime());
            expect(result.next).toBe(new Date('2026-03-24T03:00:00').getTime());
        });
    });
