...
Les deux workflows utilisent Node.js 22.x, Bun et les versions récentes des actions GitHub (`checkout@v5`, `setup-node@v6`, `setup-bun@v2`).
...
Produit un installateur dans `dist/` :

- `Tradiz-<version>-win-x64.exe` — pour les PC 64-bit

Copier le fichier sur le POS (via clé USB ou partage réseau) et l'exécuter.
