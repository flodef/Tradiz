...

{
    ...
    if [[ "$DELETE_DATA" == "yes" ]]; then
        ...
        echo "-- ============================================================"
        echo "-- NOTE: once harden-nf525-postgres.sql is applied, the append-only"
        echo "-- triggers block DELETE on balance_history (and on customers via the"
        echo "-- RESTRICT FK). Run this as the owner role with triggers disabled:"
        echo "--   ALTER TABLE dc_pos.balance_history DISABLE TRIGGER ALL;"
        echo "--   (re-enable afterwards with ENABLE TRIGGER ALL)"
