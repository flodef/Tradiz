...
CREATE TABLE IF NOT EXISTS dc.rel_formula_element_formula (
    ...
    ordre INTEGER NOT NULL DEFAULT 0,
    FOREIGN KEY (id_formula) REFERENCES dc.formulas(id) ON DELETE CASCADE,
    FOREIGN KEY (id_formula_element) REFERENCES dc.formula_elements(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS dc.rel_formula_element_product (
    ...
    ordre INTEGER NOT NULL DEFAULT 0,
    FOREIGN KEY (id_formula_element) REFERENCES dc.formula_elements(id) ON DELETE CASCADE,
    FOREIGN KEY (id_product) REFERENCES dc.products(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS dc.rel_order_formula_element (
    ...
    options TEXT,
    kitchen_view BOOLEAN NOT NULL DEFAULT false,
    FOREIGN KEY (id_order_formula) REFERENCES dc.rel_order_formula(id) ON DELETE CASCADE,
    FOREIGN KEY (id_formula_element) REFERENCES dc.formula_elements(id) ON DELETE CASCADE,
    FOREIGN KEY (id_product) REFERENCES dc.products(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS dc_pos.discounts (
    ...
    currency_id INTEGER,
    FOREIGN KEY (currency_id) REFERENCES dc_pos.currencies(id) ON DELETE SET NULL
);
